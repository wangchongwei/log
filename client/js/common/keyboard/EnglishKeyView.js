...
class MyClass extends React.PureComponent {
   ...
   */
  changeBigOrSmall =() => {
    this.setState({ type: this.state.type === 'small' ? 'big' : 'small' });
  }
   ...
   * @memberof MyClass
   */
  _renderRow =(data: Array<Object>) => {
    const singleWidth = (width - 20) / 10;
    return (
        ...
        {data.map((item, index) => {
          return (
            <SingleKey
              ...
              keyName={this.state.type === 'small' ? item.small : item.big}
            />);
        })}
      </View>
    );
  ...
  render() {
    const englishList = KeyType.englishKey;
    const englishListData = utils.getRandomArray(englishList, 10)
    const firstRowListData = englishListData.selectedArr
    const leftEnglishListData = utils.getRandomArray(englishListData.leftArr, 9)

    const secondRowListData = leftEnglishListData.selectedArr
    const threeRowListData = leftEnglishListData.leftArr
    return (
      <View style={styles.container}>
        {this._renderFirst(firstRowListData)}
        {this._renderTwo(secondRowListData)}
